...
void print_portfolio(const portfolio_t& portfolio) {
	std::for_each(portfolio.begin(), portfolio.end(), [](decltype(*begin(portfolio))& pt){ pt->print(std::cout); });
}
...
std::vector<ppricer_t> get_pricers(
    ...
    std::transform(
	    ...
	    [base_ccy](decltype(*begin(portfolio)) &pt) -> ppricer_t { return pt->pricer(base_ccy); });
    return pricers;
}
...
std::vector<std::pair<string, portfolio_values_t>> compute_pv01(
    ...
    for (const auto& d : base) {
        ...
        std::transform(pv_up.begin(), pv_up.end(), pv_dn.begin(), pv01.back().second.begin()
			, [dr](decltype(*begin(pv_up))& hi, decltype(*begin(pv_dn))& lo) -> trade_value_t { return pv01_or_nan(hi, lo, dr); });
    }
...
std::vector<std::pair<std::string, portfolio_values_t>> compute_pv01_parallel(
    ...
    auto mkt_rf = mkt.get_risk_factors(ir_rate_prefix + "([0-9]+(D|W|M|Y)\\.)?[A-Z]{3}");
    std::vector<std::string> risk_ccys;
    find_all_risk_ccy(mkt_rf, &risk_ccys);
    Market tmpmkt(mkt);
    for (const auto& risk_ccy : risk_ccys) {
        auto base = mkt.get_risk_factors(ir_rate_prefix + "([0-9]+(D|W|M|Y)\\.)?" + risk_ccy);
        ...
        pv01.push_back(std::make_pair("parallel " + ir_rate_prefix + risk_ccy, 
            std::vector<trade_value_t>(pricers.size())));
        std::transform(pv_up.begin(), pv_up.end(), pv_dn.begin(), pv01.back().second.begin(),
            [dr](decltype(*begin(pv_up))& hi, decltype(*begin(pv_dn))& lo) ->
...
std::vector<std::pair<std::string, portfolio_values_t>> compute_pv01_bucketed(
    ...
    for (auto& rf : base) {
        ...
        pv01.push_back(std::make_pair("bucketed " + bucket, std::vector<trade_value_t>(
            pricers.size())));
        std::transform(pv_up.begin(), pv_up.end(), pv_dn.begin(), pv01.back().second.begin(),
            [dr](decltype(*begin(pv_up))& hi, decltype(*begin(pv_dn))& lo) ->
            trade_value_t { return pv01_or_nan(hi, lo, dr); });
...
std::vector<std::pair<std::string, portfolio_values_t>> compute_fx_delta(
    ...
    for (const auto& risk_ccy : risk_ccys) {auto risk_factors = 
        mkt.get_risk_factors(fx_spot_prefix + risk_ccy);
        ...
        tmpmkt.set_risk_factors(risk_factors);
        fx_delta.push_back(std::make_pair(fx_spot_prefix + risk_ccy,
            std::vector<trade_value_t>(pricers.size())));

        std::transform(pv_up.begin(), pv_up.end(), pv_dn.begin(), 
            fx_delta.back().second.begin(), [dr](decltype(*begin(pv_up))& hi,
                decltype(*begin(pv_dn))& lo) ->
            trade_value_t { return pv01_or_nan(hi, lo, dr); });
    }
    return fx_delta;
}
