...
PricerForward::PricerForward(
    const TradeFXForward& trd, const std::string& base_currency)
    : m_amt(trd.quantity()),
      m_strike(trd.strike()),
      m_currency1(trd.currency1()),
      m_currency2(trd.currency2()),
...
double PricerForward::price(Market& m, const FixingDataServer* fds) const {
  ptr_disc_curve_t df = m.get_discount_curve(ir_curve_discount_name(m_currency2));
  double disc_factor = df->df(m_settle_date);

  double fwd_rate = nan<double>();
  if (fds && m.today() >= m_fixing_date) {
    const auto& fixing_name = fx_spot_name(m_currency1, m_currency2);
  ...
  if (std::isnan(fwd_rate)) {
    ...
    ptr_fx_fwd_curve_t fwd = m.get_fx_fwd_curve(fx_fwd_name(m_currency1, m_currency2));
  ...
  double fx_spot = 
    m.get_fx_spot(fx_spot_name(m_currency2, m_base_currency));
